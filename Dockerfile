FROM ubuntu:latest
MAINTAINER Qin Li < liqin@hotmail.sg >
...
RUN ln -snf /usr/share/zoneinfo/$TZ /etc/localtime && echo $TZ > /etc/timezone && \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends --no-install-suggests -y install flex bison rrdtool librrds-perl libmailtools-perl libsocket6-perl librrd-dev lighttpd  php7.4 php7.4-fpm php7.4-cli zlib1g nginx libbz2-1.0 && \
    mkdir -p /run/php/ && \
